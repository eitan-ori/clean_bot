include "map_builder.lua"
include "trajectory_builder.lua"

options = {
  map_builder = MAP_BUILDER,
  trajectory_builder = TRAJECTORY_BUILDER,
  map_frame = "map",
  tracking_frame = "base_link", -- The frame in which the robot's pose is tracked. This should be the same as the frame of the IMU data if you are using an IMU.
  published_frame = "base_link",
  odom_frame = "odom",
  provide_odom_frame = true,
  publish_frame_projected_to_2d = true,
  use_odometry = false,
  use_nav_sat = false,
  use_landmarks = false,
  num_laser_scans = 1,
  num_multi_echo_laser_scans = 0,
  num_subdivisions_per_laser_scan = 1,
  num_point_clouds = 0,
  lookup_transform_timeout_sec = 0.2,
  submap_publish_period_sec = 0.5,
  pose_publish_period_sec = 2e-2,
  rangefinder_sampling_ratio = 1.,
  odometry_sampling_ratio = 1.,
  fixed_frame_pose_sampling_ratio = 1.,
  imu_sampling_ratio = 0.5, --IMU data is often very high frequency, so we can reduce the sampling ratio to save computational resources.
  landmarks_sampling_ratio = 1.,
  trajectory_publish_period_sec = 0.10,
}

MAP_BUILDER.use_trajectory_builder_2d = true

-- Trust the laser over the wheels
TRAJECTORY_BUILDER_2D.use_imu_data = false
TRAJECTORY_BUILDER_2D.min_range = 0.1
TRAJECTORY_BUILDER_2D.max_range = 10.0
TRAJECTORY_BUILDER_2D.missing_data_ray_length = 5.0
TRAJECTORY_BUILDER_2D.use_online_correlative_scan_matching = true
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.linear_search_window = 0.05
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.translation_delta_cost_weight = 10.0
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.rotation_delta_cost_weight = 0.1

-- Submaps constraints
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 70
TRAJECTORY_BUILDER_2D.submaps.grid_options_2d.resolution = 0.02

-- Fast backend loop closure
POSE_GRAPH.optimize_every_n_nodes = 40 
POSE_GRAPH.constraint_builder.min_score = 0.65

return options
