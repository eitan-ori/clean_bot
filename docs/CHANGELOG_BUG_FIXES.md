...
### 3. Uninitialized attributes in `frontier_explorer.py`
...
### 4. Auto-start timer fired repeatedly instead of once
- **File:** `clean_bot_mission/clean_bot_mission/full_mission.py`
...
### 5. Cleaning hardware not deactivated on `go_home` / `reset`
- **File:** `clean_bot_mission/clean_bot_mission/full_mission.py`
...
### 6. Dead code: `_velocity_to_pwm` in `arduino_driver.py`
...
### 7. Unused `ekf_config` variable in launch file
...
### 9. Unused imports in `full_mission.py`
- **File:** `clean_bot_mission/clean_bot_mission/full_mission.py`
